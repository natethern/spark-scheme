...
(library airglow-3d-util
	 ...
	 (define (texparam->int p)
	   (if (integer? p)
		 ...
		 (else (raise-exception "texparam->int" "Invalid param." 
					null)))))
	 ...
	 (define (texparamval->int v)
	   (if (or (integer? v)
		 ...
		 (else (raise-exception "texparamval->int" "Invalid param." 
					null)))))
	 ...
	 (define (capability->int cap)
	   (if (integer? cap)
		 ...
		 ((post-color-matrix-color-table) 
		  spark.opengl::GL-POST-COLOR-MATRIX-COLOR-TABLE)
		 ((post-convolution-color-table) 
		  spark.opengl::GL-POST-CONVOLUTION-COLOR-TABLE)
		 ...
		 (else (raise-exception "capability->int" "Invalid capability." 
					null)))))
	 ...
	 (define (depthfunc->int f)
	   (if (integer? f)
		 ...
		 (else (raise-exception "depthfunc->int" "Invalid depth function." 
					null)))))
	 ...
	 (define (polyface->int face)
	   (if (integer? face)
		 ...
		 (else (raise-exception "polyface->int" "Invalid polyface." 
					null)))))
	 ...
	 (define (polymode->int m)
	   (if (integer? m)
		 ...
		 (else (raise-exception "polymode->int" "Invalid polymode." 
					null)))))
	 ...
	 (define (listmode->int m)
	   (if (integer? m)
		 ...
		 (else (raise-exception "listmode->int" "Invalid listmode." 
					null)))))
	 ...
	 (define (fog-param->int p)
	   (if (integer? p)
		 ...
		 (else (raise-exception "fog-param->int" "Invalid fog param." 
					null)))))
	 ...
	 (define (fog-coord->const c)
	   (if (or (integer? c)
		 ...
		 (else (raise-exception "fog-coord->const" "Invalid fog-coord." 
					null))))))
